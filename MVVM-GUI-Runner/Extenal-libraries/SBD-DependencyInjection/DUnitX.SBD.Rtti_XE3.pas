...
uses
  System.SysConst, System.Types, System.Variants,
...
{$IFDEF AUTOREFCOUNT}
    tkClass: Result := FData.FValueData = nil;
{$ELSE  AUTOREFCOUNT}
...
{$IFNDEF NEXTGEN}
            Result := ATypeInfo^.Name = 'bool';
{$ELSE}
            Result := True;  // Can't reference Name
{$ENDIF}
  end;
...
begin
  Result := DUnitX.SBD.Rtti_XE3.IsManaged(Handle);
end;
...
begin
  ...
  if ReturnType <> nil then
    Result := DUnitX.SBD.Rtti_XE3.Invoke(code, argList, CallingConvention, ReturnType.Handle, IsStatic)
  else if IsConstructor then
    Result := DUnitX.SBD.Rtti_XE3.Invoke(code, argList, CallingConvention, parList[0].ParamType.Handle, IsStatic)
  else
    Result := DUnitX.SBD.Rtti_XE3.Invoke(code, argList, CallingConvention, nil); // Invoke returns TValue.Empty
end;
...
begin
  ...
  if ReturnType <> nil then
    ...
    Result := DUnitX.SBD.Rtti_XE3.Invoke(code, argList, CallingConvention, cls.ClassInfo, IsStatic)
  else
    Result := DUnitX.SBD.Rtti_XE3.Invoke(code, argList, CallingConvention, nil);
end;
...
begin
  ...
  if ReturnType <> nil then
    Result := DUnitX.SBD.Rtti_XE3.Invoke(CodeAddress, argList, CallingConvention, ReturnType.Handle, IsStatic)
  else
    Result := DUnitX.SBD.Rtti_XE3.Invoke(CodeAddress, argList, CallingConvention, nil);
end;
...
begin
  ...
  if ReturnType <> nil then
    Result := DUnitX.SBD.Rtti_XE3.Invoke(meth.Code, argList, CallingConvention, ReturnType.Handle, False) // not static
  else
    Result := DUnitX.SBD.Rtti_XE3.Invoke(meth.Code, argList, CallingConvention, nil);
end;
...
begin
  ...
  if ReturnType <> nil then
    Result := DUnitX.SBD.Rtti_XE3.Invoke(code, argList, CallingConvention, ReturnType.Handle)
  else
    Result := DUnitX.SBD.Rtti_XE3.Invoke(code, argList, CallingConvention, nil);
end;
...
// CPUARM raw invocation logic that expects:
// ...
function Invoke(CodeAddress: Pointer; const Args: TArray<TValue>;
  CallingConvention: TCallConv; AResultType: PTypeInfo; IsStatic: Boolean): TValue;
begin
...
begin
  if FReadMethod = nil then
    GetAccessors;
  if Pointer(FReadMethod) = Pointer(1) then
    Result := nil
  else
    Result := FReadMethod;
end;
...
begin
  if FWriteMethod = nil then
    GetAccessors;
  if Pointer(FWriteMethod) = Pointer(1) then
    Result := nil
  else
    Result := FWriteMethod;
end;
