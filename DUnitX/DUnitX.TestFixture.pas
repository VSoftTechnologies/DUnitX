  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (attribute.ClassType <> TestCaseSourceAttribute) and (((attribute.ClassType = TestAttribute) and TestAttribute(attribute).Enabled) or ((attribute.ClassType <> TestAttribute) and  (method.Visibility = TMemberVisibility.mvPublished))) then
          begin
            testcases := TAttributeUtils.FindAttributes(attributes,TestCaseSourceAttribute);
            if length(testCases) > 0 then
            begin
              for testCase in testcases do
              begin
                sourceMethod := rType.GetMethod(TestCaseSourceAttribute(testCase).SourceMethodName);
                if sourceMethod <> nil then
                begin
                  if SourceMethod.ReturnType.Handle <> TypeInfo(TValueArray) then
                    raise Exception.Create('Invalid method signature on ' + sourceMethod.Name);
                  meth.Code := sourceMethod.CodeAddress;
                  meth.Data := FFixtureInstance;
                  newTest := TDUnitXTestCase.Create(FFixtureInstance,Self,TestCaseSourceAttribute(testCase).Name, method.Name,method,TTestCaseSourceFunction(meth));
                  FTests.Add(newTest);
                end;
              end;             
            end
