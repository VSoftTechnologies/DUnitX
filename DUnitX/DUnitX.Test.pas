...
type
  ...
  private
    FCaseName : string;
    FTestCaseSourceFunction : TTestCaseSourceFunction;
  ...
  public
    constructor Create(const AInstance : TObject; const AFixture : ITestFixture; const ACaseName : string; const AName : string; const AMethod : TRttiMethod; const ASourceFunction : TTestCaseSourceFunction);reintroduce;
  end;
...
constructor TDUnitXTestCase.Create(const AInstance : TObject; const AFixture: ITestFixture; const ACaseName : string;
                                   const AName: string; const AMethod: TRttiMethod; const ASourceFunction: TTestCaseSourceFunction);
begin
  inherited Create(AFixture,AName,nil);
  FInstance := AInstance;
  FRttiMethod := AMethod;
  FTestCaseSourceFunction := ASourceFunction;
  FCaseName := ACaseName;
end;
...
begin
  FStartTime := Now();
  try
    args := FTestCaseSourceFunction();
    FRttiMethod.Invoke(FInstance,args);
  finally
    FEndTime := Now();
    FDuration := TTimeSpan.Subtract(FEndTime,FStartTime);
  end;
