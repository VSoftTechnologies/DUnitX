...
type
  ...
  ///  See TestCaseSourceAttribute
  TValueArray = array of TValue;

  ///  The TestCaseSourceAttribute allows you to specify the name of a function that returns a
  ...
  private
    ...
    FCaseName : string;
  public
    constructor Create(const ACaseName : string; const ASourceMethodName : string);
    property Name : string read FCaseName;
    property SourceMethodName : string read FSourceMethodName;
  end;


  //Test Case Source functions must have this signature.
  TTestCaseSourceFunction = function : TValueArray of object;
  ...
  public
    procedure Log(const logType : TLogType; const msg : string);overload;
    procedure Log(const msg : string);overload;
    //for backwards compatibilty with DUnit tests.
    procedure Status(const msg : string);
    //redirects WriteLn to our listeners.
  ...
  ITestLogger = interface
    ...
    //Called at the start of testing. The default console listener prints the DUnitX banner.
    ...
    procedure OnLog(const logType : TLogType; const msg : string);
  ...
  ICommandLine = interface
    ...
    //procedure RegisterOptionType(const option : string; const

    //standard options
    property HideBanner : boolean read GetHideBanner; // -b
    property LogType : TLogType read GetLogType;  // -l
  end;
  ...
  public
    ...
    class function CreateRunner(const AListener : ITestLogger) : ITestRunner;overload;
    class function CreateRunner(const useCommandLineOptions : boolean; const AListener : ITestLogger) : ITestRunner;overload;
...
begin
  ...
  begin
    ...
    raise EAssertionFailure.Create(Format('left %s but got %s',[leftValue.AsStringEx,rightValue.AsStringEx]));
  end;
end;
...
begin
  ...
  begin
    leftValue := TValue.From<T>(left);
    rightValue := TValue.From<T>(right);
    raise EAssertionFailure.Create(Format('left %s Not Equal To %s',[leftValue.AsStringEx,rightValue.AsStringEx]));
  end;
end;
...
class function TDUnitX.CreateRunner(const AListener: ITestLogger): ITestRunner;
begin
  result := TDUnitXTestRunner.Create(false,AListener);
...
class function TDUnitX.CreateRunner(const useCommandLineOptions: boolean; const AListener: ITestLogger): ITestRunner;
begin
  result := TDUnitXTestRunner.Create(useCommandLineOptions,AListener);
end;
...
constructor TestCaseSourceAttribute.Create(const ACaseName : string; const ASourceMethodName: string);
begin
  FCaseName := ACaseName;
  FSourceMethodName := ASourceMethodName;
end;
...
begin
  Self.Log(TLogType.ltInformation,msg);
end;

procedure TTestFixtureHelper.Log(const logType : TLogType; const msg: string);
var
...
begin
  Self.Log(TLogType.ltInformation,'');
end;
