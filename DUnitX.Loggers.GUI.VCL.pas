...
const
  ...
  cTestResultImageMap: array[TTestResultType] of integer = (1, 2, 2, 0, 2);
...
procedure TGUIVCLTestRunner.OnTestingEnds(const RunResults: IRunResults);
  ...
  procedure PruneOrExpand(const Node: TTreeNode; const Text: string; const Expand: boolean);
  begin
    if Node.Count = 0 then
      Node.Delete
    else
    begin
      Node.Text := Format(Text, [Node.Count]);
      if Expand then
        Node.Expand(True);
    end;
  end;
...
begin
  ...
  begin
    case Test.ResultType of
      TTestResultType.Pass   : ParentNode := FPassedNode;
      TTestResultType.Ignored: ParentNode := FIgnoredNode;
    else
...
begin
  ...
  FFailureNode := tvwResults.Items.Add(nil, '');
  FFailureNode.ImageIndex := cTestResultImageMap[TTestResultType.Failure];
  FFailureNode.SelectedIndex := FFailureNode.ImageIndex;
  ...
  FMemoryLeakNode.SelectedIndex := FMemoryLeakNode.ImageIndex;

  FIgnoredNode := tvwResults.Items.Add(nil, '');
  FIgnoredNode.ImageIndex := cTestResultImageMap[TTestResultType.Ignored];
  FIgnoredNode.SelectedIndex := FIgnoredNode.ImageIndex;

  FPassedNode := tvwResults.Items.Add(nil, '');
  FPassedNode.ImageIndex := cTestResultImageMap[TTestResultType.Pass];
  FPassedNode.SelectedIndex := FPassedNode.ImageIndex;
...
begin
  actRun.Enabled := tvwTests.Items.Count > 0;
  actRunSelected.Enabled := tvwTests.Selected <> nil;
